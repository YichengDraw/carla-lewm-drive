    ...
    <main id="content">
      ...
      <section id="phases" data-kind="all">
        ...
        <details open>
          ...
          <p>当前动作：远程 SSH 恢复后，先用 <code>configs/eval_d1_route6_perception_lane_keep_ft37_robustfilter_slow_lg080_hg060_tick_1km.yaml</code> 评估 <code>ft35 retry best</code>。如果仍在约 250m lane invasion，就启动 <code>ft37 BCE-sign</code>。如果 ft37 仍失败，再采样 hard-boundary failure frames 进入 <code>ft36 temporal_delta_hardboundary</code> fallback。</p>
        </details>
        <details open>
