...
<body>
    ...
    <main id="content">
      <section id="verdict" data-kind="all">
        ...
        <div class="metrics">
          ...
          <div class="metric"><span>当前主线</span><strong>running</strong><span><code>appliednext tailgate</code>：对齐模型实际闭环动作的失败帧训练</span></div>
          ...
          <div class="metric"><span>新数据</span><strong>329</strong><span>aligned applied-action DAgger；22.5% 超过 1m 偏移</span></div>
          <div class="metric"><span>Batch</span><strong>256</strong><span>RTX 5090 峰值 24.39GB</span></div>
        </div>
        <div class="callout">
          当前判断：这还不是天气鲁棒性或 ViT 容量问题。闭环失败发生在 10-20m，且 trace 明确显示模型把大正 lane offset 预测成负数，从而持续往错误方向打方向。最新 epoch001 tailgate checkpoint 已经学到大多时候往纠偏方向打，但 lane 数值仍接近常数，尚未进入 CARLA 100m gate。
        </div>
      </section>
      ...
      <section id="evidence" data-kind="eval">
        ...
        <table>
          ...
          <tr><td>aligned applied-action DAgger</td><td>329 帧；<code>|lane_offset| &gt; 0.5m</code> 占 41.0%；<code>|lane_offset| &gt; 1m</code> 占 22.5%。</td><td>动作标签来自下一 trace row 的模型实际 applied action，和状态转移对齐。</td></tr>
      ...
      <section id="wandb" data-kind="train">
        ...
        <table>
          ...
          <tr>
            <td><a href="https://wandb.ai/yicheng132024-southern-university-of-science-technology/carla-lewm-drive/runs/d1_tiny_h3_fs5_auxpred_lane_perception_clsmean_wide_noroute_rollout_dagger_contr-20260525-111653-42200fc9"><code>d1_tiny_h3_fs5_auxpred_lane_perception_clsmean_wide_noroute_rollout_dagger_controlsign_appliednext_tailgate_6k</code></a></td>
            <td><code>appliednext</code> 表示 DAgger 动作按下一行 applied action 对齐；<code>tailgate</code> 表示失败帧高 repeat，并保存每个 validation checkpoint 供尾部筛选。</td>
            <td><span class="tag warn">running</span> epoch001 已出 checkpoint；tail sign 高但 lane corr 仍为负，继续等后续 epoch。</td>
          </tr>
          <tr>
  ...
  --config configs/train_d1_tiny_auxpred_lane_perception_clsmean_wide_noroute_rollout_dagger_controlsign_appliednext_tailgate_6k.yaml</code></pre>
        <h3>离线 tail diagnostic</h3>
        <pre><code>PYTHONPATH=src .venv/bin/python scripts/rank_aux_tail_checkpoints.py \
  --train-config /tmp/analyze_appliednext_fail_only.yaml \
  --eval-config configs/eval_d1_city_free_drive_model_rollout_lane_keep_clsmean_wide_noroute_rollout_dagger_controlsign_appliednext_tailgate_1km_simple.yaml \
  --checkpoint-glob "outputs/d1_tiny_h3_fs5_auxpred_lane_perception_clsmean_wide_noroute_rollout_dagger_controlsign_appliednext_tailgate_6k/epoch*.pt" \
  --mode rollout --split all --batch-size 64 --max-batches 16</code></pre>
        <h3>100m gate</h3>
        <pre><code>PYTHONPATH=src .venv/bin/python -m carla_lewm_drive.closed_loop_eval.evaluate \
  --config configs/eval_d1_city_free_drive_model_rollout_lane_keep_clsmean_wide_noroute_rollout_dagger_controlsign_appliednext_tailgate_1km_simple.yaml \
  --route-cap-m 100</code></pre>
      ...
      <section id="gates" data-kind="eval">
        ...
        <table>
          ...
          <tr><td>离线 tail</td><td>正/负大偏移都有样本；control sign 高；lane/heading 不是常数坍缩。</td><td>若仍坍缩，启动 <code>tailgate_thr002_6k</code>，把 active threshold 降到 0.02。</td></tr>
