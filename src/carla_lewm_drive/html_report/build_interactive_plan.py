    ...
    <main id="content">
      <section id="verdict" data-kind="all">
        ...
        <div class="metrics">
          ...
          <div class="metric"><span>Best closed-loop</span><strong>190m</strong><span>tiny delta throttle-only 通过</span></div>
          <div class="metric"><span>Next</span><strong>steering</strong><span>lane keeping 待修</span></div>
        </div>
        <div class="callout ok">
          当前状态：数据和训练流水线可信；tiny/small delta-progress + predicted-aux 在隔离 CARLA 2100 端口和 throttle-only 简化目标下 190m 无 hard infraction。200m 边界仍约 191.7m，steering-safe 150m 在 103-108m 离路。small 没有解决瓶颈，下一步是 steering-safe/lane keeping。
        </div>
      </section>
      ...
      <section id="wandb" data-kind="train">
        ...
        <table>
          ...
          <tr>
            <td><a href="https://wandb.ai/yicheng132024-southern-university-of-science-technology/carla-lewm-drive/runs/d0_small_h3_fs5_delta_predaux_e5-20260522-054348-da4e9fd5"><code>d0_small_h3_fs5_delta_predaux_e5</code></a><br>当前 small，delta-progress + predicted-aux，训练中。</td>
      ...
      <section id="phases" data-kind="all">
        ...
        <details open data-kind="eval"><summary>Phase 4：简化闭环 sanity <span class="tag good">190m pass</span></summary>
          <p>目标：用 IFD 和 Mini Driving Score 检查模型能否短程不出错。</p>
          <p>结果：隔离 CARLA + throttle-only 下 tiny/small delta-pred-aux 都 190m pass；200m 在约 191.7m off-road。steering-safe 150m 在 103-108m off-road，下一步需要恢复安全 steering。</p>
        </details>
...
python -m carla_lewm_drive.closed_loop_eval.evaluate \
  ...
  --route-cap-m 190 \
  --output-dir outputs/d0_eval_tiny_delta_predaux_throttle_only_190m</code></pre>
      </section>

      <section id="gates" data-kind="risk">
        <h2>Go / No-Go</h2>
        <div class="grid">
          <div class="callout ok"><strong>Continue</strong><br>修 steering-safe/lane keeping，然后复跑 150m steering-safe 和 200m throttle-only。</div>
