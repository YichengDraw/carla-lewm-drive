...
<body>
  <header>
    ...
    <p>D1-A 已完成第一轮 no-aux tiny 闭环评估：数据和训练链路成立，但模型仍在 30m 左右离路；多天气和更大 ViT 暂停。</p>
  </header>
  <div class="toolbar">
    ...
    <main id="content">
      <section id="verdict" data-kind="all">
        ...
        <div class="metrics">
          ...
          <div class="metric"><span>D1 total-best</span><strong>23.19m</strong><span>6/6 off-road，0% primary success</span></div>
          <div class="metric"><span>D1 action-best</span><strong>29.77m</strong><span>6/6 off-road，0% primary success</span></div>
          <div class="metric"><span>Next gate</span><strong>diagnose</strong><span>先查动作解码/数据恢复，不扩天气或模型</span></div>
        </div>
        <div class="callout bad">
          当前结论：D1-A 小范围任务没有成功。失败不是红灯、碰撞或限速，而是 action policy 输出近似固定的小左转，车辆逐渐离开车道。D1-W 多天气混训和 ViT small 先暂停，下一步做动作分布、BC baseline、recovery data / route command 诊断。
        </div>
      </section>
      ...
      <section id="live" data-kind="train">
        ...
        <div class="callout bad">截至 2026-05-23 19:26 Asia/Shanghai，训练已 early-stop，测试指标已写入；远端修复后完成 D1-A 闭环评估。<code>outputs/d1_eval_tiny_core_action_noaux_model_action_total_policyfix_500m</code> 的 mean Safety IFD 是 23.19m；<code>outputs/d1_eval_tiny_core_action_noaux_model_action_best_action_policyfix_500m</code> 是 29.77m。两者都是 6/6 off-road，primary safety success 为 0%。</div>
      </section>
      ...
      <section id="matrix" data-kind="all">
        ...
        <table>
          ...
          <tr><td>5</td><td><code>D1-W weather mix</code></td><td>D1-A 成立后，用 Clear/Cloudy/Wet/SoftRain 多天气混训并按天气分开评估。</td><td><span class="tag warn">paused</span></td></tr>
          <tr><td>6</td><td><code>train_d1_tiny_core_action_aux_ablation_20k.yaml</code></td><td>不作为主故事；动作策略诊断后再决定是否跑工程消融。</td><td><span class="tag warn">paused</span></td></tr>
          ...
          <tr><td>8</td><td><code>ViT small</code></td><td>tiny 已有闭环信号后再放大；保持同一 no-aux 目标和同一评估套件。</td><td><span class="tag warn">paused</span></td></tr>
        </table>
      </section>
      ...
      <section id="scaleout" data-kind="all">
        <h2>扩展路线</h2>
        <div class="grid">
          <div class="callout bad"><strong>小范围未过</strong><br>D1-A no-aux tiny 的 action-best 只有 29.77m，6/6 off-road。当前先修任务、数据、动作解码。</div>
...
PYTHONPATH=src .venv/bin/python -m carla_lewm_drive.closed_loop_eval.evaluate \
  ...
  --output-dir outputs/d1_eval_tiny_core_action_noaux_model_action_500m</code></pre>
      ...
      <section id="gates" data-kind="risk">
        <h2>Go / No-Go</h2>
        <div class="grid">
          <div class="callout bad"><strong>Current stop</strong><br>D1 数据和 W&B 链路成立，但 no-aux tiny 在 D1-A 上 total-best 23.19m、action-best 29.77m，6/6 off-road；不进入 D1-W 或 ViT small。</div>
